...
class Gaussians:
    ...
    def compute_cov_3D(self, quats: torch.Tensor, scales: torch.Tensor):
        ...
        if self.is_isotropic:
            s = scales.squeeze(-1)
            S = pytorch3d.transforms.Scale(x = s)
        else:
            S = pytorch3d.transforms.Scale(x = scales[:, 0], y = scales[:, 1], z = scales[:, 2])
    ...
    def compute_cov_2D(
        ...
        # of this function?
        W = getattr(camera, "R", None) # (N, 3, 3) # get_world_to_view_transform
    ...
    def evaluate_gaussian_2D(points_2D: torch.Tensor, means_2D: torch.Tensor, cov_2D_inverse: torch.Tensor):
        ...
        # Difference term: (N, H*W, 2)
        xm = points_2D - means_2D
        tmp = torch.matmul(xm, cov_2D_inverse)     # (N, H*W, 2)
        quad = (tmp * xm).sum(dim=-1)              # (N, H*W)
        power = -0.5 * quad  # (N, H*W)
...
class Scene:
    ...
    def compute_depth_values(self, camera: PerspectiveCameras):
        ...
        world_to_view = camera.get_world_to_view_transform()
        z_vals = world_to_view.transform_points(self.gaussians.means)[:, 2]
        return z_vals
    ...
    def get_idxs_to_filter_and_sort(self, z_vals: torch.Tensor):
        ...
        sorted_order = torch.argsort(z_vals[valid_mask])
    ...
    #     # Add this right at the start of compute_alphas
    #     print(f"compute_alphas called with N={len(opacities)}, img_size={img_size}")
    #     print(f"GPU memory allocated: {torch.cuda.memory_allocated()/1024**3:.2f} GB")
    #     print(f"GPU memory reserved: {torch.cuda.memory_reserved()/1024**3:.2f} GB")
        
    #     W, H = img_size

    #     # Use cached points_2D instead of recreating
    #     points_2D = self._get_points_2D(img_size)  # (1, H*W, 2)
    ...
    #     alphas = (opacities.unsqueeze(-1) * exp_power).reshape(-1, H, W)
    ...
    def render(
        ...
        # the implementation more memory efficient but at the same time makes it slower.
            ...
            # Store intermediate results that will be recomputed during backward
            image_parts = []
            depth_parts = []
            mask_parts = []
            
            start_transmittance = None
            ...
            for b_idx in range(num_mini_batches):
                ...
                # Store parts (these keep gradients but are small)
                image_parts.append(image_)
                depth_parts.append(depth_)
                mask_parts.append(mask_)
            
            # Sum at the end - this creates one unified computation graph
            image = sum(image_parts)
            depth = sum(depth_parts)
            mask = sum(mask_parts)
    ...
    def calculate_gaussian_directions(self, means_3D, camera):
        ...
        camera_centers = camera.get_camera_center().expand(means_3D.shape[0], -1)
        gaussian_dirs = means_3D - camera_centers  # (N, 3)
        return torch.nn.functional.normalize(gaussian_dirs)
